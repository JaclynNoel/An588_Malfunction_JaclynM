...
Z.prop.test <- function(p1, n1, p2 = NULL, n2 = NULL, p0, 
  ...
  if (one.sample) {
    ...
    # AZ: See note below for concerns about this equation
...
> AZ: It seems as though your test is returning different values for the two-sample test depending on the value of p0.  I believe that this is because you have p0 in your equation for the two-sample Z.  I also had some confusion about this because the equation is shown like that somewhere in the Modules.  However, it doesn't really make sense to have because we should't need a p0 value at all to compare two samples.  When I got rid of p0 in the equation for Z above, our functions returned the same results.  Apart from that, everything looks good, and is much more compact than my function!
...
> AZ: Unless I'm missing something, the dataset is named "d" here but then referred to as "data" for the remainder of the code?
...
> AZ: Our graphs look similar, but I don't see the equations on either of them.  This is strange, as I see the lines where you set it up to appear.  It might work if you use geom_text() instead, which is slightly simpler anyway!
...
```
> AZ: I like how clearly you printed the values!  I just called the summary and interpreted the results in text outside of the chunk.
...
> AZ: Again, very nice presentation!
...
> AZ: Overall, your code looks great!  You present your results very clearly and are more efficient in many places than I was.  Apart from the notes on p0 in question 1 and the missing equations on your plots, I don't see any areas of concern.
